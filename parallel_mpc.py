...
if __name__ == "__main__":
	...
	# # Lin
	# p1 = mp.Process(target=mpc, args=(0,n_agents,[3,-5,3])) 
	# p2 = mp.Process(target=mpc, args=(1,n_agents,[0,-5,3])) 
	# p3 = mp.Process(target=mpc, args=(2,n_agents,[-3,-5,3])) 
	# p4 = mp.Process(target=mpc, args=(3,n_agents,[-3,5,3])) 
	# p5 = mp.Process(target=mpc, args=(4,n_agents,[0,5,3])) 
	# p6 = mp.Process(target=mpc, args=(5,n_agents,[3,5,3]))


	####### 2 agent numsim case
	p1 = mp.Process(target=mpc, args=(0,n_agents,[8,8,5],is_collision,im_done,0,dir_name,sol_array)) 
	p2 = mp.Process(target=mpc, args=(1,n_agents,[-8,-8,5],is_collision,im_done,0,dir_name,sol_array)) 

	# # creating processes # args=(my_ID, n_agents, [final pos])
	# p1 = mp.Process(target=mpc, args=(0,6,[5,0,5],is_collision,im_done,0,dir_name,sol_array))
	# p2 = mp.Process(target=mpc, args=(1,6,[6.5,2.6,5],is_collision,im_done,0,dir_name,sol_array))
	# p3 = mp.Process(target=mpc, args=(2,6,[8,0,5],is_collision,im_done,0,dir_name,sol_array))
	# p4 = mp.Process(target=mpc, args=(3,6,[-8,0,5],is_collision,im_done,0,dir_name,sol_array))
	# p5 = mp.Process(target=mpc, args=(4,6,[-6.5,2.6,5],is_collision,im_done,0,dir_name,sol_array)) 
	# p6 = mp.Process(target=mpc, args=(5,6,[-5,0,5],is_collision,im_done,0,dir_name,sol_array))

	# ##creating processes # args=(my_ID, n_agents, [final pos])
	# p1 = mp.Process(target=mpc, args=(0,6,[4,-6,5],is_collision,im_done,0,dir_name,sol_array)) 
	# p2 = mp.Process(target=mpc, args=(1,6,[0,-6,5],is_collision,im_done,0,dir_name,sol_array)) 
	# p3 = mp.Process(target=mpc, args=(2,6,[-4,-6,5],is_collision,im_done,0,dir_name,sol_array)) 
	# p4 = mp.Process(target=mpc, args=(3,6,[-4,6,5],is_collision,im_done,0,dir_name,sol_array)) 
	# p5 = mp.Process(target=mpc, args=(4,6,[0,6,5],is_collision,im_done,0,dir_name,sol_array)) 
	# p6 = mp.Process(target=mpc, args=(5,6,[4,6,5],is_collision,im_done,0,dir_name,sol_array)) 


######### smaller hexagon
	# creating processes # args=(my_ID, n_agents, [final pos])
	# p1 = mp.Process(target=mpc, args=(0,n_agents,[2.5,-4.33,3])) 
	# p2 = mp.Process(target=mpc, args=(1,n_agents,[-2.5,-4.33,3])) 
